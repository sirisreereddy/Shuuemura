/**ADD BILLING ADDRESS AT CHECKOUT AS GUEST USER
 * @author SURYA
 *
 */
...
public class UQSU72 extends BaseTest
{
public UQSU72() 
{
	super("UQSU72");
...
public UQSU72(String testName, String browser) {
...
public void before() {
	...
	test = TestReportGenerator(report, this.getClass().getSimpleName(),"ADD BILLING ADDRESS AT CHECKOUT AS GUEST USER");
}
...
public void test() throws Exception {
	...
	if(channel.equalsIgnoreCase("Desktop")){
		...
		objData.setTestData();
		test.log(LogStatus.PASS,"TEST CASE NAME:ADD BILLING ADDRESS AT CHECKOUT AS GUEST USER");
		Reporter.log("TEST CASE NAME:ADD BILLING ADDRESS AT CHECKOUT AS GUEST USER");
		...
		cp.enterBillingDetails(objData.fname,objData.lname,objData.Billingaddress,objData.billing_city,objData.billing_zipcode,objData.phone,objData.billing_State);
		cp.clickContinueToShipping();
		cp.clickSaveAndContinueBtnInShippingAddrPopUp();
		//cp.clickSaveAndContinueBtnInShippingAddrPopUp();
		//cp.clickCreditCardRbtn();
		cp.validateShippingMethods();
		
		cp.verifyBillingAddressInReviewOrderPage(objData.Billingaddress);
		homePageObject=null;
		cp=null;
		shoppingBag=null;
		System.gc();
